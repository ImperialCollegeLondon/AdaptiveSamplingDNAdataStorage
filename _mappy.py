...
from readfish.plugins.local_align import local_aligner, align_top
...
class _Aligner(AlignerABC):
    ...
    def __init__(self, mappy_impl: Aligners, debug_log: Optional[str] = None, **kwargs):
        ...
        self.local_aligner = local_aligner()
    ...
    def _c_mappy_wrapper(self, basecalls: Iterable[Result]) -> Result:
        ...
        """
        from mappy import Alignment
        # TODO: fill in from the config file
        #reflist = [("0", "AGTGCAACAAGTCAATCCGT"),("1","AACAAGACTTTCGGAGCGTT")] #file0
        reflist = [("0", "TCGAAAGCGTGGACAAACAA"),("1","AAGCCGAATTAAAGCGCACT")] #file1
        #reflist = [("0", "TGCCAATAAGTCTTGCGGAA"),("1","CGAAATCAAGGAACGGTGGA")] #file2
        reflist_with_revcomp = []

        for ref_name, ref in reflist:
            import string
            old_chars = "ACGT"
            replace_chars = "TGCA"
            tab = str.maketrans(old_chars,replace_chars)
            ref_revcomp = ref.translate(tab)[::-1]
            reflist_with_revcomp.append((ref_name, ref, ref_revcomp))
        for result in basecalls:
            #print(self.aligner.__doc__)
            #print(f"SEQ: {result.seq}")
            #self.logger.error(f"SEQ: {result.seq}")
            #print(self.aligner_params)

            result.alignment_data = []
            for ref_name, ref, ref_revcomp in reflist_with_revcomp:
                ## Full matches only
                # ref_len = len(ref)
                # match_pos_start = result.seq.find(ref)
                # if match_pos_start != -1:
                #     match_pos_end = match_pos_start + ref_len
                #     match_forw = +1 # -1 if reverse
                #     al = Alignment(ref_name, ref_len, 0, 19, match_forw, match_pos_start, match_pos_end, 0, "", 1, 1, 1, 0, 0, 0, "", "")
                #     result.alignment_data.append(al)

                # TODO: Handle non-exact matches
                score_threshold = 70

                al = align_top(ref, result.seq, aligner=self.local_aligner)
                ...
                if score >= score_threshold:
                    match_forw = +1 # -1 if reverse
                    al = Alignment(ref_name, target_end - target_start, target_start, target_end, match_forw, chunk_start, chunk_end, 0, "", 1, 1, 1, 0, 0, 0, "", "")
                    result.alignment_data.append(al)

                al = align_top(ref_revcomp, result.seq, aligner=self.local_aligner)
                score, aligned_seq1, pattern, aligned_seq2, _alignment = al
                coords = _alignment.aligned
                target_start, target_end = coords[0,0,0], coords[0,-1,1]
                chunk_start, chunk_end = coords[1,0,0], coords[1,-1,1]
                ...
                #print(result.alignment_data)
            #for al in result.alignment_data:
            #    print(al)
            #result.alignment_data = list(self.aligner.map(result.seq))
            # if len(result.alignment_data) > 0:
            #     print(result.seq)
            yield result
